...
const getFoodsData = () => {
  getFoods().then((res) => {
    ...
    category.value = [...new Set(ca)]
    // 获取纸巾大类下面所以商品
    foodsList.value = data.filter((i) => {
      return i.category === '纸巾'
    })
    // 获取推荐商品
    recommendList.value = data.filter((i) => {
...
const changCategory = (item) => {
  current.value = item
  foodsList.value = allData.value.filter((i) => {
    return i.category === item
  })
}

// 购物车
const allFoodsNumber = ref(0)
const totalFoodsPrice = ref(0)
const ifProductDetail = ref(false)
const { cartData, addCart } = useCartStore()
...
<template>
  <div class="choose">
    ...
    <div class="recommends">
      <div class="all-box">
        <div class="food-box" v-for="(item, index) in recommendList" :key="index">
          <div class="pic">
    ...
    <div class="footer">
      ...
      <div class="finish" v-show="allFoodsNumber > 0" @click="toPay">选好了</div>
    </div>
    <!--购物清单-->
